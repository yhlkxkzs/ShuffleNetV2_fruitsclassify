#!/usr/bin/env python3
"""对单张或多张图片做水果分类推理（ShuffleNet V2，与训练时一致的预处理）。"""
...
DEFAULT_WEIGHTS = REPO_ROOT / "models" / "shufflenet_fruit_cls_best.pt"
IMAGE_EXT = {".jpg", ".jpeg", ".png", ".webp", ".bmp", ".ppm", ".tif", ".tiff"}

VARIANTS = {
    "x0_5": (shufflenet_v2_x0_5, ShuffleNet_V2_X0_5_Weights.IMAGENET1K_V1),
    "x1_0": (shufflenet_v2_x1_0, ShuffleNet_V2_X1_0_Weights.IMAGENET1K_V1),
}
...
def load_model(weights_path: Path, device: torch.device):
    ...
    model = builder(weights=weights_enum)
...
def main() -> int:
    p = argparse.ArgumentParser(description="ShuffleNet V2 水果分类推理")
    p.add_argument("--weights", type=Path, default=DEFAULT_WEIGHTS, help="checkpoint 路径")
    p.add_argument("--incoming", type=Path, default=REPO_ROOT / "incoming", help="待识别图片目录")
    p.add_argument("--output", type=Path, default=REPO_ROOT / "output" / "predictions.json", help="输出 JSON")
    p.add_argument("images", nargs="*", type=Path, help="可选：指定图片路径；留空则扫描 --incoming")
    args = p.parse_args()
